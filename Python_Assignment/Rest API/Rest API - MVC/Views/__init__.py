...
class OrderView(Resource):
    ...
    def put(self,OrderID:str):
        ...
        if order:
            ...
            OrderDate = datetime.strptime(data['OrderDate'], '%Y-%m-%dT%H:%M:%S.%f')
            RequiredDate = datetime.strptime(data['RequiredDate'], '%Y-%m-%dT%H:%M:%S.%f')
            ShippedDate = datetime.strptime(data['ShippedDate'], '%Y-%m-%dT%H:%M:%S.%f')

            order = OrderModel(OrderID,data["CustomerID"],data["EmployeeID"],OrderDate,
            RequiredDate,ShippedDate,data["ShipVia"],data["Freight"],
